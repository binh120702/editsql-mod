...
def read_data_json(split_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  ...
  for interaction_data in split_data:
    ...
    for turn in interaction_data['interaction']:
      ...
      special_chars = ["'", '"']
      turn_sql_w_values = [] 
      query_toks = sqlparse.format(turn['query'], reindent=True).replace('\n', ' ').replace('(', ' ( ').replace(')', ' ) ').replace('>', ' > ').replace('<', ' < ').replace(',', ' , ').replace('\\', '').split()
      index = -1
      open_quote = False
      
      if turn['query'] == "SELECT project_id from Project_staff where staff_id  =  '5137097.0'":
        x = 0
        
      while index < len(query_toks) - 1:
        index += 1
        query_tok = query_toks[index]
        
        is_quote = False
        for special_char in special_chars:
          if special_char in query_tok:
            if query_tok[0] == query_tok[len(query_tok) - 1] == special_char and len(query_tok) > 1:
              turn_sql_w_values.append(query_tok)
              is_quote = True
              break
            
            values_in_quote = [query_tok]
            index += 1
            if index >= len(query_toks):
              break
            
            query_tok = query_toks[index]
            
            while special_char not in query_tok and index < len(query_toks):
              values_in_quote += [query_tok]
              index += 1
              query_tok = query_toks[index] 
              
            values_in_quote += [query_tok] 
            query_tok = ' '.join(values_in_quote) 
            turn_sql_w_values.append(query_tok)
            is_quote = True
        ...
        if query_tok != '.' and '.' in query_tok:
          if query_tok.replace('.', '').isnumeric():
            turn_sql_w_values.append(query_tok)
            continue
          
          turn_sql_w_values += query_tok.replace('.', ' . ').split()
          
          if ',' in query_tok:
            last_tok = turn_sql_w_values[len(turn_sql_w_values) - 1]
            turn_sql_w_values.pop()
            turn_sql_w_values += last_tok.replace(',', ' , ').split()
          continue
        ...
        if len(turn_sql_w_values) == 0:
          turn_sql_w_values.append(query_tok)
        if query_tok != turn_sql_w_values[len(turn_sql_w_values) - 1] or len(query_tok) == 1:
          turn_sql_w_values.append(query_tok)
      ...
      if len(turn_sql) != len(turn_sql_w_values):
        print('turn_sql', turn_sql)
        print('turn_sql_w_values', turn_sql_w_values)
      
      for idx, tok in enumerate(turn_sql):
        turn_sql_w_values[idx] = turn_sql_w_values[idx].replace('%','')
        if tok == 'value':
          turn_sql[idx] = turn_sql_w_values[idx]
          
      turn_sql = ' '.join(turn_sql)
      turn_sql = turn_sql.replace('"', ' " ')
      turn_sql = turn_sql.replace("'", " ' ").strip()
