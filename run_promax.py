...
def infer_promax(model, data, params, split):
...
def infer_with_model(params, model):

    st = time()
    # Prepare the dataset into the proper form.
    data = atis_data.ATISDataset(params)
    print("INFERENCE - LOAD DATA IN :", time() - st)

    infer_promax(model, data, params, split='valid')
